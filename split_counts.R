split_counts <- function (counts, log_data = FALSE, exclude_extreme = FALSE, return_cutoff = FALSE, return_mean = FALSE, return_cutoff_mean = FALSE,  mu= c(0.3, 0.65), label_name = NULL) {
  ...
  best_bic <- Inf
  for (k in 2:4) {
    model <- normalmixEM(x = x, k = k, arbvar = FALSE, maxit = 10000, maxrestarts = 30, epsilon = 1e-20)
    n <- length(counts)
    kv <- length(model$lambda) + 2*length(model$mu) # Number of parameters
    aic <- -2*model$loglik + 2*kv
  ...
  # Continue with best_model
  
  # Example: Plotting for the best model (simplified, adapt as necessary)
  x_df <- as.data.frame(x)
  plot_1 <- ggplot(x_df, aes(x = x)) + 
    geom_histogram(aes(y = ..density..), binwidth = 0.1, fill = "grey", color = "black", alpha = 0.5) +
    stat_function(fun = function(v) sum(sapply(1:best_model$k, function(i) dnorm(v, mean = best_model$mu[i], sd = sqrt(best_model$sigma[i])) * best_model$lambda[i])), color = "blue") +
    labs(title = "Best Fit: Histogram with Gaussian Mixture Model", x = "Value", y = "Density") +
    theme_minimal()  
  
  print(plot_1)
  return(best_model)
  
  model <- normalmixEM(x= x, k=2, mu = mu, lambda =c(.6, .4),sigma = c(0.3, 0.2), arbvar = FALSE ,  maxit =10000, maxrestarts = 30, epsilon = 1e-20)
