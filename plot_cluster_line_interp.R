plot_cluster_line_interp <- function (expr_list, meta_list, gene2cluster, p_time = NULL, 
                                     ylimits = c(-2,2), include_n = TRUE, col_num = 4, 
                                     label_len = 20, text_size = 16, order_clust = FALSE, 
                                     ribbon = TRUE, x_label = FALSE, add_t50 = NULL) {
...
#print
  ...
  if (!is.null(add_t50)) {
    ...
    add_t50 <- add_t50 %>%
      ...
      dplyr::summarise(mean_avg_t50 = mean(avg_t50))
  }
  print(head(add_t50))
  ...
  mean_expr$cluster_id <- factor(gsub("_", " ", mean_expr$cluster_id))
 ...
 # mean_expr$cluster_id <- factor(mean_expr$cluster_id, levels = levels(mean_expr$cluster_id)[first_ind])
  ...
  if (!is.null(add_t50)) {
    p <- p + geom_point(data = add_t50, aes(x = mean_avg_t50, y = 0), 
                        #inherit.aes = FALSE,
                        colour = "black", 
                        fill = "black" ) + 
  ...
  p <- p + coord_cartesian(ylim=ylimits) +
    ...
    scale_x_continuous(labels = p_time)
  ...
  if (x_label) {
    p <- p + theme(axis.text.x = element_text(size=text_size-4)) 
  } else {
